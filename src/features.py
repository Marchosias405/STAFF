# src/features.py
# -----------------------------------------------------------------------------
# Build structured + text features for HS-code classification.
# Uses:
...
def _clean_basic(df: pd.DataFrame) -> pd.DataFrame:
    req = ["id", "description", "tags", "price", "weight",
           "origin", "dest", "gift", "label_id"]
    ...
    for c in ["tags", "origin", "dest", "description"]:
...
def make_splits(df: pd.DataFrame, label_col: str = "label_id", seed: int = 42):
    ...
    can_stratify_full = (vc.min() >= 2)

    if can_stratify_full:
        # First split: stratified train vs temp
        ...
        print("[features] Warning: some classes have <2 samples; using non-stratified first split.")
        train_df, temp = train_test_split(
            df,
            test_size=0.30,
            random_state=seed,
    ...
    # Now check if the TEMP split can be stratified
    vc_temp = temp[label_col].value_counts()
    can_stratify_temp = (vc_temp.min() >= 2)
    ...
    if can_stratify_temp:
        ...
        print("[features] Warning: temp set has classes with <2 samples; using non-stratified second split.")
        valid_df, test_df = train_test_split(
            temp,
            test_size=0.50,
            random_state=seed,
...
def fit_transform_and_cache(csv_path: str, out_prefix: str):
    ...
    # Inputs: NOW includes description text
    used_cols = ["tags", "description", "price_bin",
                 "weight_bin", "origin", "dest", "gift"]
